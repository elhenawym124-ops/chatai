...
const AlertsPanel: React.FC<AlertsPanelProps> = ({ className = '' }) => {
  ...
  const resolveAlert = async (alertId: string) => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/v1/monitor/alerts/${alertId}/resolve`, {
        method: 'POST'
      });
  ...
  const triggerCheck = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3001/api/v1/monitor/alerts/check', {
        method: 'POST'
      });
