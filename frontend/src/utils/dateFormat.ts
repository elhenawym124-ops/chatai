...
// Date formatting utilities with support for Gregorian and Hijri calendars
...
export const DATE_FORMAT_CONFIGS = {
  ...
  hijri: {
    locale: 'ar-SA-u-ca-islamic', // Arabic locale with Islamic calendar
    options: {
      year: 'numeric' as const,
      month: 'long' as const,
      day: 'numeric' as const,
...
export const formatDate = (
  ...
  try {
    ...
    const options = customOptions || config.options;
...
export const isValidDateFormat = (format: string): format is DateFormatType => {
  return format === 'gregorian' || format === 'hijri';
};
