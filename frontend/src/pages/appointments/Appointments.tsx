...
const Appointments: React.FC = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/v1/appointments?${queryParams}`);
  ...
  const fetchAvailableSlots = async () => {
    try {
      const response = await fetch(
        `http://localhost:3001/api/v1/appointments/available-slots?staffId=${newAppointment.staffId}&date=${newAppointment.appointmentDate}`
      );
      const data = await response.json();
