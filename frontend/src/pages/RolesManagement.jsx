...
const RolesManagement = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = editModalOpen 
        ? `http://localhost:3001/api/v1/companies/${user.companyId}/roles/${selectedRole.key}`
        : `http://localhost:3001/api/v1/companies/${user.companyId}/roles`;
  ...
  const handleDeleteRole = async (roleKey, role) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/v1/companies/${user.companyId}/roles/${roleKey}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
          'Content-Type': 'application/json'
