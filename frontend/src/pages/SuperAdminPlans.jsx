...
const SuperAdminPlans = () => {
  ...
  const fetchPlans = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3001/api/v1/admin/plans/plans', {
        headers: {
  ...
  const fetchComparison = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch('http://localhost:3001/api/v1/admin/plans/plans/comparison', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleSavePlan = async (planData) => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`http://localhost:3001/api/v1/admin/plans/plans/${selectedPlan.planType}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
