...
router.post('/generate-response', requireAuth, aiController.generateResponse);

// Analysis endpoints
router.post('/analyze-sentiment', requireAuth, aiController.analyzeSentiment);
router.post('/recommend-products', requireAuth, aiController.recommendProducts);
router.get('/insights/:conversationId', requireAuth, aiController.getConversationInsights);

// Analytics (managers and admins only)
router.get('/analytics', requireAuth, requireRole(['COMPANY_ADMIN', 'MANAGER']), aiController.getAnalytics);

// Settings management (managers and admins only)
router.get('/settings', requireAuth, requireRole(['COMPANY_ADMIN', 'MANAGER']), aiController.getSettings);
router.put('/settings', requireAuth, requireRole(['COMPANY_ADMIN', 'MANAGER']), aiController.updateSettings);

// Testing (managers and admins only)
router.post('/test', requireAuth, requireRole(['COMPANY_ADMIN', 'MANAGER']), aiController.testResponse);

// ==================== ADVANCED AI ROUTES ====================

// Model management
router.get('/models', requireAuth, requireRole(['COMPANY_ADMIN', 'MANAGER']), aiController.getAvailableModels);
router.get('/usage-stats', requireAuth, requireRole(['COMPANY_ADMIN', 'MANAGER']), aiController.getUsageStats);

// Advanced response generation
router.post('/generate-advanced-response', requireAuth, aiController.generateAdvancedResponse);

// ==================== PROMPT MANAGEMENT ROUTES ====================

// Prompt templates
router.get('/prompt-templates', requireAuth, requireRole(['COMPANY_ADMIN', 'MANAGER']), aiController.getPromptTemplates);

// Company prompts
router.get('/prompts', requireAuth, requireRole(['COMPANY_ADMIN', 'MANAGER']), aiController.getCompanyPrompts);
router.put('/prompts', requireAuth, requireRole(['COMPANY_ADMIN', 'MANAGER']), aiController.updateCompanyPrompts);
