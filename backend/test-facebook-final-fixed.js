...
async function testFacebookFinalFixed() {
  console.log('🔍 اختبار نهائي مُحدث لأمان Facebook Settings...\n');
  ...
  try {
    ...
    for (const api of readAPIs) {
      try {
        ...
        if (hasCompanyId1 && hasCompanyId2) {
          ...
          console.log(`   - شركة 1: ${response1.data.companyId} (متوقع: ${user1.companyId})`);
          console.log(`   - شركة 2: ${response2.data.companyId} (متوقع: ${user2.companyId})`);
          allIsolated = false;
        }
    ...
    const page1Data = {
      pageId: `final-test-1-${Date.now()}`,
      pageAccessToken: 'token-final-test-1',
      pageName: 'صفحة اختبار نهائي - شركة 1'
    };

    const page2Data = {
      pageId: `final-test-2-${Date.now()}`,
      pageAccessToken: 'token-final-test-2',
      pageName: 'صفحة اختبار نهائي - شركة 2'
    };
    ...
    try {
      ...
      if (company1Pages.length > 0 && company2Pages.length > 0) {
        const page1Id = company1Pages.find(p => p.pageName.includes('نهائي'))?.pageId;
        const page2Id = company2Pages.find(p => p.pageName.includes('نهائي'))?.pageId;
      ...
      try {
        const testPage1 = company1Pages.find(p => p.pageName.includes('نهائي'));
        const testPage2 = company2Pages.find(p => p.pageName.includes('نهائي'));
